...
export default function TaskList() {
    ...
    return (
      <div className="flex justify-center items-center min-h-[240px]">
        <div className="animate-pulse flex flex-col items-center gap-2">
          <div className="h-6 w-6 rounded-full border-2 border-gray-200 border-t-blue-500 animate-spin"></div>
          <p className="text-gray-500 text-sm">Loading task lists...</p>
        </div>
      </div>
    );
  }
  ...
  if (!taskLists.length) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[240px] text-gray-500">
        <NewspaperIcon className="w-12 h-12 mb-2 text-gray-300" />
        <h3 className="text-lg font-semibold mb-1">No Task Lists Found</h3>
        <p className="text-sm">There are no task lists available at the moment.</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl w-full min-h-screen mx-auto p-4">
      ...
      <div className="grid gap-3">
        {taskLists?.map((taskList) => (
          <Card 
            key={taskList._id}
            className="group cursor-pointer shadow-none hover:shadow-sm transition-all duration-300 overflow-hidden"
          ...
          >
            <div className="p-3 flex items-center justify-between bg-white">
              ...
              <ChevronRightIcon className="w-4 h-4 text-gray-400 transition-transform group-hover:translate-x-1" />
            </div>
          </Card>
        ))}
      </div>
